...
type
  ...
  public
    class function AmbienteEstaAcessivel: boolean; static;
...
class function TIntegracaoAPIClass.AmbienteEstaAcessivel: boolean;
const
  URL_API = 'https://af3tqle6wgdocsdirzlfrq7w5m.apigateway.sa-saopaulo-1.oci.customer-oci.com/fiap-sandbox/health';
var
  lRestClientHealthCheck: TRestClient;
  lRestRequestHealthCheck: TRestRequest;
  lRestResponseHealthCheck: TRestResponse;
begin
  lRestClientHealthCheck := TRestClient.Create(URL_API);
  lRestResponseHealthCheck := TRestResponse.Create(nil);

  lRestRequestHealthCheck := TRestRequest.Create(nil);
  lRestRequestHealthCheck.Method := rmGET;
  lRestRequestHealthCheck.Client := lRestClientHealthCheck;
  lRestRequestHealthCheck.Response := lRestResponseHealthCheck;

  lRestRequestHealthCheck.Params.AddHeader('Authorization', TIntegracaoAPIClass.RetornarChaveAuth2);

  lRestRequestHealthCheck.Params[0].Options := [poDoNotEncode];
